...
import { deleteIntegrationSecret, setIntegrationSecret } from "@/lib/integration-secrets"
...
export async function POST(request: Request) {
  ...
  try {
    ...
    await setIntegrationSecret(user.id, "finance", "session_cookie", result.sessionCookie)
    await deleteIntegrationSecret(user.id, "finance", "access_token")
